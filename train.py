...
def load_dict(args, generator, G_optimizer, discriminator, D_optimizer):
        ...
        model_pth = getattr(args, "home_dir", os.path.join("Checkpoint", args.logdir))
...
def train(args, global_step, train_loader, generator, G_optimizer, recon_loss, w_progression, discriminator, D_optimizer, HOME_DIR, real_label=None, fake_label=None, criterion=None):
    ...
    scan_name = getattr(args, "scan_name", "scan_t1ce")
    crop_pad_key = f"{scan_name}_crop_pad"
    noisy_key    = f"{scan_name}_noisy"
    ...
    for step, batch in enumerate(train_loader):
        ...
        for D_n_i in range(args.D_n_update):
            ...
            x_crop_pad = batch[crop_pad_key][0:args.batch_size].cuda() # Scan cropped and with padding to 96x96x96
            x_crop_noisy = batch[noisy_key][0:args.batch_size].cuda() # Scan cropped, with padding to 96x96x96 and noise
        ...
        for G_n_i in range(args.G_n_update):
            #batch = next(iter(train_loader))
            # Get data from the batch
            x_crop_pad = batch[crop_pad_key][args.batch_size:].cuda() # Scan cropped and with padding to 96x96x96
            x_crop_noisy = batch[noisy_key][args.batch_size:].cuda() # Scan cropped, with padding to 96x96x96 and noise
...
def __main__():
    ...
    if args.checkpoint_dir == "":
        HOME_DIR = os.path.join("Checkpoint", args.logdir)
    else:
        HOME_DIR = os.path.join(args.checkpoint_dir, args.logdir)
